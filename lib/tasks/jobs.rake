namespace :jobs do
  desc "Run this script to remove duplicate ScrapeProductVariantUrlJobs (with the same url)"
  task clear_scrape_product_url_duplicates: :environment do
    puts "Removing duplicate ScrapeProductVariantUrlJobs (with the same url)..."
    ...
    result = ActiveRecord::Base.connection.execute("
      ...
      where serialized_params->>'arguments' IN (
        ...
        from good_jobs
        where job_class = 'ScrapeProductVariantUrlJob' and performed_at is null
        group by serialized_params->>'arguments'
        having count(serialized_params->>'arguments') > 1
      )")

    if result.cmd_tuples > 0
      puts "Removed #{result.cmd_tuples} duplicate ScrapeProductVariantUrlJobs (with the same url)"
    else
      puts "No duplicate ScrapeProductVariantUrlJobs (with the same url) found"
    end
  end
